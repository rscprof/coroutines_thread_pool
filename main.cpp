...
requires (T range) {
  requires addable<std::ranges::range_value_t<T>>;
};

template<sized_random_access_range_with_addable_items RANGE>
task<std::ranges::range_value_t<RANGE>> calc(RANGE v, const awaitable_factory& factory,decltype(v.size()) threshold = 10000) {
  auto size = v.size();
  if (size < threshold) {
